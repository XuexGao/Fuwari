---
title: "Get Timestamps for NetEase Cloud Music Songs"
description: "If you need lyrics timestamps and the song is provided by NetEase Cloud Music, obtaining them will be incredibly simple."
category: "Record"
...
To fetch a song's lyrics with timestamps from NetEase Music, extract the song ID from the share link, then append it to `https://music.163.com/api/song/media?id=XXX`. The returned JSON includes the lyrics with precise timecodes. This method bypasses the need for apps or web interfaces, directly accessing the API for raw lyric data.
:::

# Formally begin

First, we need to obtain the ID of a song. If you are using the App, click Share, then `Copy Link`. If you are on the Web, simply copy the URL from the browser address bar.

You will receive a URL similar to this one.

`https://music.163.com/#/song?id=29812004&uct2=U2FsdGVkX1+0Ic0zSL7yd7aZMm+xya0r1rUmiRUO5Us=`

The value in `id=29812004` is our song ID.
...
Visit to get lyrics timestamps!
