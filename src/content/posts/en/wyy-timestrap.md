---
title: "NetEase Cloud Music Song Timestamp Retrieval"
description: "If you require song timestamps, and the song is available on NetEase Cloud Music, obtaining them will be straightforward."
category: "Record"
...
The article provides a URL to a song ID, followed by a command-line tool that retrieves the song’s lyrics and audio information.  It then uses this data to create a JSON object containing song status, lyric version, and lyrics. The output includes a series of unsettling statements about an impending danger and a character's struggle against unseen enemies.
:::

# Formal commencement.

First, we need to obtain the song ID. If you are using an app, you can share it and then `copy link`.  If it’s a web page, simply copy your browser address bar.

You will receive a similar URL.

`https://music.163.com/#/song?id=29812004&uct2=U2FsdGVkX1+0Ic0zSL7yd7aZMm+xya0r1rUmiRUO5Us=`

The ID of our song is `id=29812004`.
...
Get access to lyric timestamps.
