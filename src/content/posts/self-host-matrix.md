...
1. **安装 Matrix:** 首先需要安装并配置 Synapse (Matrix) 手建立托管环境。
2. **创建数据库:**  在 Synapse 中创建一个名为 `synapse` 的 PostgreSQL 数据库，并指定其主键为 `synapse`。
3. **配置服务器:**  通过 `homeserver.yaml` 文件配置服务器的名称、公共 URL 和 PID 文件。
4. **部署 PostgreSQL:**  安装并创建名为 `synapse` 的用户账户，并设置所有者为 `synapse`。
5. **删除数据库:** 移除刚刚创建的 `synapse` 数据库。
6. **重新创建数据库:** 创建同名数据库，并设置所有者为 `synapse`。
7. **启用联邦:**  启用联邦服务，以便在多个服务器之间进行数据同步。
8. **配置邮件:** 配置邮件设置，确保SMTP验证正常，并使用 GitHub OAuth 认证。
9. **自动注册和邮箱验证:**  启用自动注册，并设置邮箱验证和密码找回机制。
10. **允许邮箱登录:** 允许用户通过邮箱登录到 Synapse。
11. **速率限制:** 设置邮箱的速率限制，防止恶意攻击。
12. **媒体存储:** 配置媒体存储，包括本地媒体文件保留时间以及远程媒体文件保留时间。
13. **删除陈旧设备:**  定期删除陈旧的设备，以提高系统性能。
14. **创建管理员账号:** 使用 `register_new_matrix_user` 命令创建一个管理员账号，并使用 `homeserver.yaml` 文件进行配置。

最后，该文章还提供了创建管理员账号的步骤和访问 App.element.io 的方法。
:::
