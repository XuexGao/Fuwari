...
*   **区域选择:** 选择了日本东部（东京）的账户主区域。
*   **安全:** 建议启用两步验证并使用移动设备扫描甲骨文App进行TOTP。
*   **映像上传:** 需要先将自定义映像上传到甲骨文账户下的对象存储，具体步骤可参考 [下载 Debian](https://www.debian.org/distrib)，需要先下载适用于云的Debian映像（64位 AMD/Intel, 64位 ARM）。
*   **实例创建:**  可以通过云端创建实例，但无法使用Debian映像。
*   **VCN (网络):** 需要创建VCN，并选择合适的镜像类型（x86 或 arm64），可以自由拆分。
*   **IPv6配置:** 可以更改IPv6地址，并根据VCN是否附加IPv6前缀进行设置。
