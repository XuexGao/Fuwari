...
# 第四个坑：那条只在切页瞬间闪出来的横线，其实不是某个组件自己的边

在把整体毛玻璃链路收稳之后，站里又冒出了一个更诡异的小问题：

- 点击 Navbar 触发 Swup 切页时；
- 首页首屏靠近 Navbar 下沿的位置，会快速闪出一条细横线；
- 页面完全切稳后，这条线又会立刻消失；
- 而且只要这条固定位置上方存在毛玻璃，线就会被显得非常明显。

最开始我也走了几次弯路。

第一反应当然是怀疑 Navbar 自己：

- 是不是 Navbar 顶部或底部有遗留补片；
- 是不是 `card-base` 的边框在中间态暴露出来了；
- 是不是公告卡片顶部那条彩色渐变边界被“折射”上来了；
- 或者是不是某个 `backdrop-filter` 在切页瞬间采样到了不该采的内容。

于是我做了很多针对性的排除实验：

- 删掉 Navbar 早期遗留的装饰补片；
- 单独去掉 Navbar 的 `backdrop-filter`；
- 单独去掉文章卡片的 `backdrop-filter`；
- 临时隐藏公告卡片，或者把公告渐变压平成普通纯色卡片；
- 甚至连整站固定背景层的 `filter`、`transform`、`background-attachment`、`isolation` 这些老高危项也都逐个试了一遍。

这些实验有些能改变闪烁的次数，有些完全无效，但有一个事实逐渐变得很清楚：

> 真正的问题并不在某一张卡片本身，而更像是某条“固定交界带”在切页时被毛玻璃显影出来了。

后来我索性不再猜某条具体 border，而是直接开始大幅删减顶部布局本身：

- 先去掉 `#top-row` 和 `#navbar-wrapper` 上那些古早的 `transition-all`；
- 再去掉 `sticky`、`top`、`navbar-offset` 这类吸顶联动；
- 最后更激进一点，直接把原先的 `#top-row -> #navbar-wrapper -> Navbar` 这一层旧壳结构拆掉，只保留一个最裸的 `#navbar-wrapper`。

结果非常直接：

- 只删一两个属性时，横线仍然存在；
- 但当旧的顶部壳结构整体被拆掉后，那条线会立刻彻底消失。
...
- 某篇文章的边框；
- 某个公告组件的渐变；
- 也不是某个单独的 blur 容器自己“长出来”的线。

更接近真实情况的是：

- 旧的顶部布局外壳里，存在一条固定的、平时不明显的布局/合成交界；
- Swup 切页时，这条交界会在某几帧里被反复重绘；
- 一旦这条固定位置上方覆盖了毛玻璃层，人眼就会把这条 seam 看成一根快速闪烁的横线。

也就是说，这其实又是一个非常典型的“古早结构遗留问题”——

> 单看任何一个组件都像没错，但老布局壳、切页时序、固定位置交界和毛玻璃叠在一起之后，浏览器就会把那条原本藏着的 seam 直接显影出来。

顺着这个结论，后面就不该再去给它打遮罩、硬延时或者继续猜公告卡片，而应该优先做两件事：

1. 保留那版已经证明能消掉横线的精简顶部结构；
2. 再从这个干净基线出发，按需一点点把真正必要的功能加回去。

这次补查对我最大的提醒反而不是“某条 CSS 写错了”，而是：

> 当一个视觉问题表现得像某个组件坏了时，真正的根因很可能藏在更上层的全局承载结构里。

另外，这轮排查里还有一个顺手确认的小结论：

- 整站背景 blur 本身依旧是一个高风险变量；
- 切页时如果主动冻结或恢复它，会让背景出现“先没模糊、再突然有模糊”的跳变；
- 所以最终背景和背景模糊都更适合作为真正的“不动产”常驻，而不是在切页过程中再去人为开关。

至于背景默认值，最后我也顺手把默认背景模糊改回了 `0`，把主动调高 blur 的权利留给用户自己。
