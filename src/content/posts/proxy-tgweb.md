...
该项目旨在为用户提供一个“immersive”的翻译体验，通过在根路径上创建新的 `index.html` 文件，并使用CSS样式来模拟翻译过程。 这种方法允许用户访问根路径上的页面，但同时也可能被恶意利用，因为它可能会导致用户访问到未预期的内容，从而引发Abuse。

该解决方案的核心在于创建一个新的 `index.html` 文件，并在其中设置一些 CSS 样式来模拟翻译效果。  这些样式主要用于创建视觉上表示翻译过程的元素，例如一个“toast”显示器，并使用CSS动画来呈现翻译的动态效果。
:::
