...
---
> [!ai] qwen/qwen3-vl-8b
> IPFS 是一种去中心化文件系统，通过内容寻址和点对点网络实现文件分发。用户可通过 IPFS Desktop 或 kubo 启动本地节点参与网络。文件上传后可通过 IPFS 网关（如 ipfs.crossbell.io）以 CID 访问。实际使用中存在节点稀少、上传延迟、带宽占用等问题，常需借助服务商如 Fleek 提升效率。Fleek 支持 Git 部署、域名绑定，并提供三种访问方式：CDN 域名（解析少）、原生 IPFS（CID 变更）、自定义域名（CDN 反代，IP 多、速度快）。尽管访问常依赖中心化代理，但用户连接后可实现 P2P 访问。Fleek 可用于静态网站部署、图床、图片存储及存算分离等场景。
