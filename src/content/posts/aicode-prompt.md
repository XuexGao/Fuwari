...
根据文章内容，以下是关于如何完成任务的简短总结：

**核心建议：** 明确需求、逐步分解、利用AI工具进行翻译和代码生成。不要盲目相信AI提供的最终结果，而是要对AI生成的代码进行验证和改进。

**具体步骤：**

1.  **清晰定义需求:** 详细描述任务目标，包括所需功能、技术栈等。
2.  **逐步分解任务:** 将复杂问题分解为更小的子任务，逐个实现。
3.  **利用AI工具:** 选择合适的AI IDE (例如 VS Code, Cursor, Trae) 进行代码翻译和自动调试，并利用AI提供的代码片段进行辅助。
4.  **验证和改进:** 对生成的代码进行全面审查，确保其功能正确、效率高，并根据实际情况进行修改和优化。
5.  **保持警惕:** 意识到AI可能出错，不要完全依赖AI的输出结果，要进行独立验证和判断。
:::
