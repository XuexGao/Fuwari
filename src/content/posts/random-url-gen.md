...
---
> [!ai] qwen/qwen3-vl-8b
> 随机图API可通过多种架构实现：传统方案需本地存储图片并承担高带宽成本；前后端分离可降低服务器压力，但后期仍需维护；云函数方案提升并发能力，但需跨服务拉取资产；最佳实践是利用云函数支持的动静结合存储。本质是服务器响应不同内容——直接返回图片或302重定向，前者节省请求次数但增加代理流量，后者适合分离架构。奇技淫巧包括：Cloudflare Origin Rules生成UUID实现无计费边缘随机URL；或前端JS直接拼接URL生成随机图，无需后端介入。三种流派：传统派（边缘函数取图）、极客派（CF规则无计费）、环保派（前端JS自动生成）。
