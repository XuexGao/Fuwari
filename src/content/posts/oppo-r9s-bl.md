...
> [!ai] claude
> OPPO R9s 刷第三方系统的完整流程分为五步：进 9008 深度刷机模式（关机后同时按电源键+音量加减连接电脑，检查设备管理器端口）、降级到可解 BL 的固件版本（使用线刷工具选择对应型号刷入）、解 BL 锁（通过 ADB 命令 `fastboot oem unlock-go` 解锁 bootloader）、刷入 TWRP Recovery（使用 XiaoMI Flash 工具在 9008 模式下刷写）、在 TWRP 中清除数据并刷入第三方 ROM。关键卡点：9008 驱动报错需手动安装驱动，降级握手失败需重进模式或检查数据线，fastboot 命令报错需在开发者选项中开启 OEM 解锁。工具与刷机包均提供网盘下载。
